...
// Resolve a path inside node_modules by walking UP from the source dir, so the
// vendored tree-sitter WASM resolves whether node_modules is co-located (kloel
// self-contained layout) or hoisted to the repo root (atomic-os package layout).
function resolveNodeModules(rel: string): string {
  let d = SOURCE_DIR;
  for (let i = 0; i < 10; i += 1) {
    const cand = path.join(d, 'node_modules', rel);
    if (fs.existsSync(cand)) return cand;
    const up = path.dirname(d);
    if (up === d) break;
    d = up;
  }
  return path.join(SOURCE_DIR, 'node_modules', rel);
}
...
async function initTreeSitter(): Promise<Record<string, unknown>> {
  ...
  const wts = await import(resolveNodeModules('web-tree-sitter/web-tree-sitter.js'));
...
async function loadLanguage(lang: string): Promise<unknown> {
  ...
  const wasmPath = resolveNodeModules(wasmRel);
