 ...
 * which reads no stdin, would otherwise be reparented to init (ppid 1) and listen
 * forever. Such orphans were the dominant atomic-exec RAM leak — and the validator
 * lattice had ZERO process/resource-lifetime coverage, so the leak was invisible
 * to it. This gate closes that class.
 *
 ...
 *             within the poll window (no orphan left behind).
 *
 * Pure: spawns into os.tmpdir(), kills only its own descendants, self-cleans.
 * Run: node gates/atomic-exec-broker-parent-reap.proof.mjs [--json]
 */
