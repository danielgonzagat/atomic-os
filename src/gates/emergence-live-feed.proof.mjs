...
//   (3) the CANONICAL observatory (evolution/emergence-observatory.mjs) consumes the REAL fed feed:
...
import { noveltyIndex, anomalyResidual, verifyResidualChain } from '../../evolution/emergence-observatory.mjs';
...
// (1) record real events on simulated operations
...
check('records are chained (each previousSha = prior recordSha)', feed[0].previousSha === null && feed[1].previousSha === s1 && feed[2].previousSha === s2);

// (2) chain verifies; tampering breaks it (discriminating)
...
// (3) the CANONICAL observatory consumes the REAL feed
...
check('O5 anomalyResidual flags the unpredicted (non-alpha) event', res.residual.length === 1);
check('O5 residual chain verifies', verifyResidualChain(res.residual).ok === true);

// (4) fail-safe + exemption
let threw = false;
let nullOnBad = null;
...
check('EXEMPT: .atomic bookkeeping write is not recorded', recordEmergenceEvent({ repoRoot: root, kind: 'edit', op: 'x', file: '.atomic/trace.json', before: '', after: 'q' }) === null);
